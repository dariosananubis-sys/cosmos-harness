...
def ejecutar(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.comando == "medir":
            resultado_medicion = medir_arbol(arbol, metodo=args.metodo or config.metodo, presupuesto=config.entrada)
        ...
        if args.comando == "compilar":
            ...
            config_efectiva = replace(config, destino_compilacion=destino, modo_compilacion=modo)
        ...
        if args.comando == "mapa":
            ...
            return 0
    except (MetodoNoDisponible, ErrorCompilacion) as exc:
