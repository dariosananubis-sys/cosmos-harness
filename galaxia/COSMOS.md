...
Veinte oficios completos y cinco aguas transversales; nada se carga hasta que se entra en ello.
...
- `descender` — Se baja al nivel que hace falta y solo a ese. Lo de al lado no se abre.
- `irreversible` — Lo que no se puede deshacer se prepara entero y se pide el visto bueno.
- `precedencia` — Cuando dos reglas chocan, gana la más específica; y una orden reciente gana a una regla.
- `secretos` — Ninguna credencial se escribe en un fichero, un prompt o un mensaje. Se leen al vuelo.
- `verificar` — Nada se declara hecho sin haberlo visto funcionar y sin haberlo visto fallar.
