...
`async-profiler`) y a `perf` por lo pragmático: corre en macOS sobre Apple Silicon sin instalar nada
más, mientras que `perf` es solo Linux y `Instruments` obliga a abrir Xcode entero.
