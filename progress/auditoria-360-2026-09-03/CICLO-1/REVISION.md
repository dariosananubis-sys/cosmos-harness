...
| F-08 | PARCIAL | `shell=True` fuera y `argv` en lista ✓. **Queda media corrección**: la línea `set VANGUARDIA_TELEGRAM_USER_ID=…` sigue igual dos líneas más abajo, y el nombre de la agencia viaja en ficheros de producto (GOAL §5) |
...
| R-39 | El nombre de la agencia (`VANGUARDIA_TELEGRAM_USER_ID`) viaja en dos ficheros de producto que este mismo fix tocó, contra GOAL §5 («fuera de cualquier organización») |
...
encontró que la segunda mitad (`set VANGUARDIA_TELEGRAM_USER_ID=…`) quedó sin corregir. La próxima
