...
| F-08 | media corrección | `OPENCLAW_TELEGRAM_USER_ID`, `export`, sin nombre de la agencia (R-39); `cosecha/` retirada (§4.3): ya no hay gemelo que olvidar | `grep VANGUARDIA` = 0 |
...
- Barrido del árbol entero (sin `.git`): rutas `/Users/<usuario>` = **0**; `Anubis`, `VanguardIA`,
  `Arnes-Dario`, `ClawArmy`, `Omnia`, `vosasesoria`, `dariosan` = **0** fuera de los informes
  originales de la auditoría (que son evidencia y no se editan). `research/VANGUARDIA-HARNESS.md` →
