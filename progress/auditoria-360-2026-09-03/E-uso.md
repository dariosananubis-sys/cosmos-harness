...
Rama `main`, remoto `https://github.com/dariosananubis-sys/cosmos-harness.git`.
...
El repo ya lo sabe, en `research/VANGUARDIA-HARNESS.md:22`:
...
El que exige Claude Code (skill real instalada en la máquina, `~/Arnes-Dario/.claude/skills/agent-browser/SKILL.md`):
...
grep -rn  "~|dariosatino"                            -> 0 ficheros
grep -rln "Anubis|VanguardIA|Arnes-Dario|anubis.es"  galaxia/         -> 0 ficheros
```
...
la máquina de Darío y cero marcas de Anubis en el árbol publicable. **Nada que retirar por este
frente.**
...
skill real: `~/Arnes-Dario/.claude/skills/agent-browser/SKILL.md`). COSMOS escribe `cosmos:`,
...
- **El contenido está limpio de esta máquina.** 0 rutas `~`, 0 marcas
  Anubis/VanguardIA en `galaxia/`, 0 topes de RAM o de concurrencia. El commit `4eb9604` cerró ese
