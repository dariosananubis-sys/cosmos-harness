...
`~/Arnes-Dario/memory/feedback-ensenar-al-escaner-no-callarlo.md:26` —
...
diff -q ~/cosmos/galaxia/pueblos/auditor-de-skills/scripts/skill_security_auditor.py \
        ~/Arnes-Dario/.claude/skills/skill-security-auditor/scripts/skill_security_auditor.py
...
```python
subprocess.run('claude -p "%_VANGUARDIA_PROMPT%" --output-format text --dangerously-skip-permissions',
...
**b) `%_VANGUARDIA_PROMPT%` es sintaxis de CMD de Windows.** En el POSIX `sh` que usa `shell=True` en
macOS y Linux **no expande nada**:

```bash
$ _VANGUARDIA_PROMPT="ordendelusuario" sh -c 'echo claude -p "%_VANGUARDIA_PROMPT%"'
claude -p %_VANGUARDIA_PROMPT%
```
...
4. **Cero credenciales o infraestructura de la agencia.** Ni IPs de producción, ni el Anuubis prod, ni
