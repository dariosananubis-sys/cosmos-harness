...
MUTACIONES = (
    ...
    # --- Revisión C (2026-09-04): el alta de máquina --------------------------------------
    Mutacion(
        "M89",
        "cosmos/configurar.py",
        "        habia, anterior = _obtener(previo or {}, clave)\n        if habia:\n            _poner(copia, clave, anterior)\n",
        "        habia, anterior = _obtener(previo or {}, clave)\n        if False:\n            _poner(copia, clave, anterior)\n",
        "tests.test_revision_c.ManualDevuelveLoQueHabia.test_c01_el_valor_previo_del_usuario_se_repone_en_vez_de_borrarse",
        "C-01: sin reponer el valor previo, `--autonomia manual` deja al usuario sin el defaultMode que tenía",
    ),
    Mutacion(
        "M90",
        "cosmos/configurar.py",
        ...
        "            _quitar(copia, clave)\n",
        "tests.test_revision_c.ManualDevuelveLoQueHabia.test_c18_un_permissions_vacio_vuelve_byte_a_byte",
        "C-18: sin reponer el padre vacío, `{\"permissions\": {}}` no vuelve byte a byte",
    ),
    Mutacion(
        "M91",
        "cosmos/configurar.py",
        ...
        "    if intacto and not guardado.get(\"existia\"):\n        if True:\n            ruta.unlink()\n",
        "tests.test_revision_c.ManualDevuelveLoQueHabia.test_c05_manual_en_seco_no_escribe",
        "C-05: `--autonomia manual --seco` que escribe",
    ),
    Mutacion(
        "M92",
        "cosmos/configurar.py",
        "    if ruta.is_symlink():\n        # `write_text` sigue el enlace",
        "    if False:\n        # `write_text` sigue el enlace",
        "tests.test_revision_c.ElLanzadorYElEnlace.test_c09_forzar_sobre_un_symlink_no_escribe_a_traves_del_enlace",
        "C-09: `--lanzador --forzar` sobre un symlink pisa el fichero al que apunta",
    ),
    Mutacion(
        "M93",
        "cosmos/cli.py",
        ...
        "    if False:\n        # `--lanzador --seco` escribía el shim (revisión C-07).\n",
        "tests.test_revision_c.SecoEsSeco.test_c07_lanzador_en_seco_no_escribe",
        "C-07: `--lanzador --seco` escribe el shim",
    ),
    Mutacion(
        "M94",
        "cosmos/cli.py",
        ...
        "    if False:\n        # `--seco` escribía perfil y credenciales igual que sin él (revisión C-08).\n",
        "tests.test_revision_c.SecoEsSeco.test_c08_el_alta_y_comprobar_en_seco_no_escriben",
        "C-08: el alta con `--seco` escribe perfil y credenciales",
    ),
    Mutacion(
        "M95",
        "puente/modelos.py",
        "    if seco:\n        if plist.is_file() and es_nuestro(plist):\n            lineas.append(f\"  (seco) descargaría",
        "    if False:\n        if plist.is_file() and es_nuestro(plist):\n            lineas.append(f\"  (seco) descargaría",
        "tests.test_revision_c.SecoEsSeco.test_c04_modelos_quitar_en_seco_no_descarga_nada",
        "C-04: `--modelos quitar --seco` descarga el agente y borra de verdad",
    ),
    Mutacion(
        "M96",
        "puente/modelos.py",
        ...
        "    if False:\n        directorio.rmdir()\n",
        "tests.test_revision_c.SecoEsSeco.test_c22_quitar_no_deja_el_bin_vacio_y_dice_que_el_log_se_conserva",
        "C-22: `quitar` deja ~/.cosmos/bin vacío y no es byte a byte",
    ),
    Mutacion(
        "M97",
        "puente/modelos.py",
        ...
        "            elif False:\n",
        "tests.test_revision_c.ElInventarioNoDiceOkSinMirar.test_c10_un_interprete_que_no_existe_no_es_ok",
        "C-10: `estado` dice ok con un intérprete del plist que no existe",
    ),
    Mutacion(
        "M98",
        "puente/modelos.py",
        "    elif not Path(partes[0]).is_file():\n        filas.append(Fila(\"hook SessionStart\", \"falta\"",
        "    elif False:\n        filas.append(Fila(\"hook SessionStart\", \"falta\"",
        "tests.test_revision_c.ElInventarioNoDiceOkSinMirar.test_c10_un_interprete_que_no_existe_no_es_ok",
        "C-10: `estado` dice ok con un hook cuyo intérprete no existe",
    ),
    Mutacion(
        "M99",
        "cosmos/estado.py",
        ...
        "    elif False:\n",
        "tests.test_revision_c.ElInventarioNoDiceOkSinMirar.test_c11_credenciales_con_permisos_flojos_no_son_ok",
        "C-11: credenciales en 644 y el inventario dice ok",
    ),
    Mutacion(
        "M100",
        "puente/modelos.py",
        "        f\"{shlex.quote(str(interprete))} {shlex.quote(str(reponedor))} >/dev/null 2>&1 &  # {MARCA}\"\n",
        "        f\"{interprete} {reponedor} >/dev/null 2>&1 &  # {MARCA}\"\n",
        "tests.test_revision_c.ElHookYElPlistConRutasRaras.test_c12_la_orden_del_hook_soporta_espacios_y_se_ejecuta",
        "C-12: el hook SessionStart con un espacio en la ruta no ejecuta nunca",
    ),
    Mutacion(
        "M101",
        "puente/modelos.py",
        "    interprete, reponedor, log = _xml(str(interprete)), _xml(str(reponedor)), _xml(str(log))\n",
        "    interprete, reponedor, log = str(interprete), str(reponedor), str(log)\n",
        "tests.test_revision_c.ElHookYElPlistConRutasRaras.test_c21_el_plist_escapa_el_xml",
        "C-21: un `&` en la ruta deja un plist que launchd no puede leer",
    ),
    Mutacion(
        "M102",
        "cosmos/guardarrailes.py",
        ...
        "    if False:\n",
        "tests.test_revision_c.ElCableadoDeSesion.test_c02_enganchar_dos_veces_conserva_el_primer_respaldo",
        "C-02: enganchar dos veces pisa el respaldo y desenganchar devuelve los hooks llamándolo byte a byte",
    ),
    Mutacion(
        "M103",
        "cosmos/guardarrailes.py",
        ...
        "    if False:\n",
        "tests.test_revision_c.ElCableadoDeSesion.test_c13_sin_respaldo_y_sin_nada_nuestro_no_se_reescribe_lo_ajeno",
        "C-13: desenganchar sin respaldo reformatea un settings.json ajeno",
    ),
    Mutacion(
        "M104",
        "cosmos/guardarrailes.py",
        ...
        "        quitado = True\n",
        "tests.test_revision_c.ElCableadoDeSesion.test_c13_podar_conserva_una_lista_vacia_del_usuario",
        "C-13: podar se lleva las listas vacías del usuario",
    ),
    Mutacion(
        "M105",
        "cosmos/guardarrailes.py",
        "    if not push.exists() or es_nuestro(push):\n        return None\n    return (f\"ya hay un pre-push ajeno",
        "    if True:\n        return None\n    return (f\"ya hay un pre-push ajeno",
        "tests.test_revision_c.ElCableadoDeSesion.test_c17_un_pre_push_ajeno_se_dice",
        "C-17: un pre-push ajeno deja el escáner sin instalar y nadie lo dice",
    ),
    Mutacion(
        "M106",
        "cosmos/cli.py",
        ...
        "        fijada = None\n",
        "tests.test_revision_c.ElAltaInteractivaRespetaLasBanderas.test_c06_con_autonomia_por_bandera_no_se_pregunta",
        "C-06: `instalar --autonomia` interactivo pregunta igual y hace otra cosa",
    ),
    Mutacion(
        "M107",
        "puente/proyectar.py",
        ...
        "        if True:\n            resultado[clave] = valor\n            continue\n",
        "puente.tests.test_proyectar.Proyeccion.test_conserva_las_claves_ajenas_de_settings",
        "C-03: proyectar pisa los ajustes del anfitrión y comprobar los impone",
    ),
    Mutacion(
        "M108",
        "puente/proyectar.py",
        "    return f\"{prefijo}{bloque_nuevo}{resto}\"\n",
        "    return f\"{prefijo}{resto}{bloque_nuevo}\"\n",
        "puente.tests.test_proyectar.Proyeccion.test_el_bloque_se_sustituye_donde_estaba",
        "C-15: el bloque se manda al final y desplaza lo que el dueño escribió después",
    ),
    Mutacion(
        "M109",
        "puente/proyectar.py",
        ...
        "puente.tests.test_proyectar.Proyeccion.test_un_claude_md_con_crlf_conserva_su_final_de_linea",
        "C-14: un CLAUDE.md con CRLF se reescribe entero en LF",
    ),
    Mutacion(
        "M110",
        "puente/proyectar.py",
        "        if hijo.is_symlink():\n            # Es del anfitrión: se ignora.",
        "        if hijo.is_symlink():\n            raise ErrorProyeccion(\"una skill del destino no puede ser symlink\")\n            # Es del anfitrión: se ignora.",
        "puente.tests.test_proyectar.Proyeccion.test_una_skill_enlazada_del_anfitrion_no_aborta_la_proyeccion",
        "C-20: una skill enlazada del anfitrión aborta la proyección entera",
    ),
    Mutacion(
        "M111",
        "cosmos/cli.py",
