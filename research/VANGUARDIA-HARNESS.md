...
FAIL: test_full_audit_is_clean
  AssertionError: [] != ['falta .agents/skills/.generated-by-vanguardia', ...87 elementos]
...
`VANGUARDIA_PACKS` tiene precedencia sobre el fichero mientras exista, y los comandos que mutan
...
`.generated-by-vanguardia` **dentro de cada carpeta de skill individual**, con un contenido exacto
...
renombra el destino actual a `.<nombre>.vanguardia-backup`, renombra el staging al nombre final, y
...
marcadores `<!-- vanguardia-harness:start -->` / `<!-- vanguardia-harness:end -->`. Si no existen y
...
el marcador propio (`HOOK_MARKER = "# managed-by-vanguardia-harness"`) o coincide con el patrón de un
...
1. **La taxonomía de 11 niveles no tiene equivalente en VanguardIA y es la razón de ser del
...
3. **Riesgos de esta vía:**
   ...
   - El presupuesto de VanguardIA es en **bytes**; el de COSMOS es en **tokens aproximados**. Si se
   ...
   - Ninguno de los scripts de VanguardIA fue probado aquí contra un repo externo real (la regla de
...
agrupación más expresivo y validado; absorber el código de VanguardIA entero traería su modelo plano
