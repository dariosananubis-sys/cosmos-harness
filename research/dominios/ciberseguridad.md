...
- `hacklcx/HFish`, `hashcat/hashcat` (cracking) y `vanhauser-thc/thc-hydra` (fuerza bruta de credenciales) — dobles usos legítimos en auditoría de contraseñas autorizada, pero fuera de "de primera/segunda" de este barrido por no encajar en ningún hueco que no cubran ya PEASS-ng/Impacket para el caso de uso de Anubis; se anotan para no repetir la búsqueda si hace falta cracking puntual.
...
4. **Suricata y Zeek** solo aparecen en Segunda fila con datos básicos (estrellas, licencia, push);
   ...
   general) — merece su propia pasada si Anubis monta monitorización de red para algún cliente.
...
6. **Herramientas de credential-cracking** (`hashcat`, `thc-hydra`) se dejaron en Humo por no encajar
   en un hueco propio de este barrido, pero si Anubis hace auditorías de contraseñas para cliente,
