...
| multi-review.py | Revisión de código multi-modelo en paralelo (Gemini CLI local + 2 modelos gratis de OpenRouter vía `asyncio.gather`), parseo de veredicto XML, consenso con exit codes, modo alterno de auditoría de frontend | agentes | Branding "VanguardIA" en headers/paths/prints; ruta ejemplo `src/<la agencia>/<la-agencia>-panel/src` → `src/mi-proyecto/frontend/src` |
